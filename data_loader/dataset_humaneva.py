...
class DatasetHumanEva(Dataset):
    ...
    def process_data(self):
        ...
        for key in list(data_f.keys()):
            print(f"[DEBUG] Subject {key} contains actions:", list(data_f[key].keys()))


if __name__ == '__main__':
    np.random.seed(0)
    actions = 'all'
    dataset = DatasetHumanEva('test', actions='Walking')
    ...
    render_animation(dataset.skeleton, data_tmp, output=f'.\{1}.gif')
